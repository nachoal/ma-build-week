...
|---|---|---|---|
| Guided state machine | Model gate → verified review session → explicit attempt → review → briefed waiter → reverified session → explicit attempt → review → completion | Replacement rerun `.build/test-results/MA-release-access-final-standard.xcresult`: 227/227 executions, 0 skipped (193 definitions plus 38 dynamic runs); the isolated private `MALive` target was absent | PASS (simulator/code) |
...
| Verified local deletion | Keychain credential is absent before any profile reset; deletion/readback failure preserves the profile and explains recovery in the active language | Simulator: focused 7/7 at `.build/test-results/MA-local-data-deletion-focused.xcresult`, also included in the complete 213/213 suite. Physical iPhone/iOS 27.0: clean 4/4 real-Keychain bundle plus clean 3/3 English/Spanish/success UI bundle with the generated `arm64` XCTest architecture explicitly selected | PASS (simulator and physical code/Keychain/UI) |
| Real Apple audio integration | One model tap completes; AVAudio capture/stop exits cleanly; labeled deterministic post-stop learner payload reaches secret-free feedback; silence fabricates neither transcript nor review | Current simulator gate starts/stops the shipping audio owner, substitutes only the simulator's often-silent learner payload, and requires feedback. Earlier physical 1/1 proved capture cleanup only and is not replacement-Release review evidence | PASS (current simulator capture/review); replacement physical archive and real-mic semantic review remain separate gates |
...
| Worker contract suite | Fixed low-reasoning Realtime/planner policies, bounded schemas, auth/rate limits, privacy guards, transient retry identity, permanent-status no-retry | `.build/test-results/MAWorker-final-freeze.tap`: 35/35, covering every retryable 408/409/500/502/503/504 and permanent 501/505; private deployment version `57d49379-af1f-4160-8e88-ec611ab9a1d7` | PASS (service) |
...
| Privacy manifest and export compliance | No tracking; declared Realtime audio/aggregate use; CA92.1; non-exempt encryption explicitly `NO` | `plutil`, unit tests, generated Info settings, and exact-commit archive recheck | PASS in final archive |
| Secret scan | Current set, staged inputs, reachable history, app bundle, and compiled executable strings | `scripts/scan-secrets.sh` after the 227/227, live stress, and focused audio gates and again inside the exact-commit archive workflow | PASS on current source; replacement archive recheck pending |
